...
triggers.post('/bling', (request, response) => {
  ...
  const updateProductAtEcomplus = async trigger => {
    let triggerEstoque = trigger.retorno.estoques[0].estoque

    // busca a variação
    let sqlVariation = 'SELECT variation_sku, parent_sku FROM ecomplus_products_variations WHERE variation_sku = ? AND store_id = ? AND lojaId = ?'
    let valuesVariation = [triggerEstoque.codigo, storeId, lojaId]
    ...
    if (produto[0].product_bling_stock !== triggerEstoque.estoqueAtual) {
      ...
      mysql.query(sqlUpdate, [triggerEstoque.estoqueAtual, produto[0].product_sku, storeId, lojaId])
        .then(async () => {
          ...
          let resource = '/products/' + produto[0].product_id + '.json'

          sdk.apiRequest(storeId, resource, 'PATCH', { quantity: triggerEstoque.estoqueAtual })
            .then(r => end())
