...
module.exports = async appSdk => {
    ...
    getStores()
      .then(stores => {
        ...
        mp.tasks(stores, function (configObj, storeId, next, current, err) {
          console.log(configObj, storeId, next, current, err)
          let retry = 0
          if ((!err && storeId)) {
                ...
                .then(({ data }) => {
                  ...
                  if (result && result.length) {
                    ...
                    Promise.all(promises)
                      .then((r) => {
                        if (count > 0) {
                          logger.log(`<< Save ${count} products in DB | #${storeId}`)
                        }
                        next()
                      })
                  }
  ...
  const start = () => save().finally(() => setTimeout(() => start(), 1 * 90 * 1000))
  // start after 30s
  setTimeout(() => start(), 1 * 40 * 1000)
}
