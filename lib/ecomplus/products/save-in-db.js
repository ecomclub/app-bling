    ...
    const callback = function (configObj, storeId, next, current, err) {
      ...
      ecomClient.store({
            ...
            if (data && data.variations && Array.isArray(data.variations) && data.variations.length) {
              ...
              variations.forEach(variation => {
                if (variation.sku) {
                  ...
                  logger.error(`! Variação ${variation._id} sem sku pra o produto ${data._id}, não salva no banco, será necessário sincronizar o produto manualmente`)
                }
              })
        ...
        return Promise.all(promises).then((r) => {
          if (success > 0) {
            logger.log(`<< Save ${success} products in DB | #${storeId}`)
          }
          return next()
        })
      }).catch(err => {
