      ...
      sdk.apiApp(storeId, null, 'GET')
        .then(async resp => {
          ...
          if (!application.hasOwnProperty('data') ||
            ...
            (application.data.hasOwnProperty('synchronize') &&
              !application.data.synchronize.hasOwnProperty('orders'))) {
            resolve()
          }

          let orders = []
          // sincronizar todos?
          if (application.data.synchronize.orders.all && application.data.synchronize.orders.all === true) {
          ...
          if (application.data.synchronize.orders.byId && application.data.synchronize.orders.byId.length > 0) {
            let orderById = application.data.synchronize.orders.byId.map(async order => {
